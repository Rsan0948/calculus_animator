...
class HelicOpsIntegration:
    ...
    def audit_workspace(self, workspace_path: Path) -> GuardrailReport:
        ...
        Args:
            workspace_path: Path to the workspace to audit.
            
        Returns:
            GuardrailReport with audit results.
            
        Raises:
            RuntimeError: If HelicOps is not available and allow_mock is False.
        """
        if not self.available:
            if self.allow_mock:
                # ONLY for explicit testing - clearly marked as fake
                logger.warning("MOCK MODE: HelicOps unavailable - returning FAKE pass for testing only")
                return GuardrailReport(
                    target_path=str(workspace_path),
                    overall_pass=True,
                    violations=[],
                    audit_summary={"mode": "MOCK - DO NOT USE IN PRODUCTION"}
                )
            raise RuntimeError(
                "HelicOps not available. Install with:\n"
    ...
    def _mock_report(self, workspace_path: Path) -> GuardrailReport:
        """DEPRECATED: Mock mode should not be used in production.
        
        Raises:
            RuntimeError: Always, to prevent accidental mock usage.
        """
        raise RuntimeError(
            "HelicOps mock mode is deprecated and should not be used in production. "
            "Install HelicOps: pip install -e ~/Desktop/HelicOps/packages/{core,py} "
            "Or explicitly set allow_mock=True only for testing."
        )
