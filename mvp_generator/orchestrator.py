...
from engine.state import GeneratedFile, GuardrailReport, MathResult, MVPAttempt, MVPOutput, Violation
...
from mvp_generator.validators import validate_all
...
class Orchestrator:
    ...
    def generate_mvp(self, math_result: MathResult, max_retries: int = 5) -> MVPOutput:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            # 4.5 Validate generated code BEFORE writing to disk
            logger.info("Step 4.5: Validating generated code...")
            pre_violations = validate_all(implemented_code)
            if pre_violations:
                logger.warning("Pre-write validation found %d issues:", len(pre_violations))
                for violation in pre_violations:
                    logger.warning("  - [%s] %s", violation.check_id, violation.message[:60])
                # Add to report for agent feedback
                if report is None:
                    ...
                    report.violations.extend(pre_violations)
            ...
            for violation in report.violations:
                logger.info("    - [%s] %s: %s", violation.severity, violation.check_id, violation.message[:80])

            attempt_history.append(
                MVPAttempt(
                    attempt_number=attempt,
                    generated_files=sorted(implemented_code.keys()),
                    overall_pass=report.overall_pass,
                    math_validation_pass=report.math_validation_pass,
                    violation_count=len(report.violations),
                    violation_check_ids=sorted({violation.check_id for violation in report.violations}),
                )
            )
            ...
            if attempt == max_retries:
                ...
                logger.info(
                    "Retry assignments: %s",
                    {key: len(value) for key, value in agent_assignments.items() if value},
                )

        # Convert to MVPOutput
        generated_files = [
            GeneratedFile(relative_path=path, content=content)
            for path, content in implemented_code.items()
        ]
    ...
    def _format_violations_for_agents(self, report: Optional[GuardrailReport]) -> str:
        ...
        for violation in report.violations:
            lines.append(f"\n[{violation.severity.upper()}] {violation.check_id}")
            if violation.file_path:
                lines.append(f"  File: {violation.file_path}:{violation.line_number or '?'}")
    ...
    def _has_architect_violations(self, report: Optional[GuardrailReport]) -> bool:
        ...
        return any(violation.check_id in architect_checks for violation in report.violations)
    ...
    def _has_tester_violations(self, report: Optional[GuardrailReport]) -> bool:
        ...
        return any(violation.check_id in tester_checks for violation in report.violations)
    ...
    def _has_integrator_violations(self, report: Optional[GuardrailReport]) -> bool:
        ...
        return any(violation.check_id in integrator_checks for violation in report.violations)
