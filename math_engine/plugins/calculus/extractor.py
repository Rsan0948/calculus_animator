...
class ExpressionExtractor:
    ...
    def extract(
        ...
        Also handles some simple natural language forms like "derivative of x^3".
        ...
        # 0. Natural language pre-cleaning
        clean_latex = latex.strip()
        
        # Strip "derivative of", "integral of", "limit of", "sum of" etc.
        m = re.match(r"(?:derivative|integral|limit|sum|solve)\s+of\s+(.*)", clean_latex, re.IGNORECASE)
        if m:
            clean_latex = m.group(1).strip()
            # If we stripped "derivative of", we can hint at the type if needed, 
            # but usually the detector handles it.

        # 1. derivative
        m = re.search(
            r"\\frac\{d(?:\^(\d+))?\}\{d([a-z])(?:\^(\d+))?\}\s*(.*)",
            clean_latex, re.DOTALL,
        )
        if m:
            order = int(m.group(1) or m.group(3) or 1)
            params.setdefault("variable", m.group(2))
            params["order"] = order
            return m.group(4).strip() or clean_latex, params
        ...
        m = re.search(r"\\int\s*(.*?)\s*d([a-z])", clean_latex, re.DOTALL)
        if m:
            params.setdefault("variable", m.group(2))
            return m.group(1).strip(), params

        # plain indefinite integral int ... dx / ∫ ... dx
        m = re.search(r"(?:∫|int)\s*(.*?)\s*d\s*([a-z])", clean_latex, re.DOTALL)
        if m:
            params.setdefault("variable", m.group(2))
            return m.group(1).strip(), params
